...
import { ArrowLeft, Send, CheckCircle2, XCircle, Loader2, ArrowRight, Flag } from "lucide-react";
...
export default function PerformanceTask({ task, onBack, onSubmit, isSubmitting, studentGradeLevel }: PerformanceTaskProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-5rem)] bg-[#F4EBD0] overflow-hidden">
      {/* Left Panel: Scenario */}
      <div className="lg:w-1/2 bg-[#FFF9E6] p-8 lg:p-12 border-r-4 border-black overflow-y-auto">
        <button
          onClick={onBack}
          className="flex items-center text-black font-bold hover:-translate-x-1 transition-transform mb-8 bg-white border-2 border-black px-4 py-2 rounded-xl shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
        >
          <ArrowLeft className="w-5 h-5 mr-2" strokeWidth={3} />
          Back to Map
        </button>
        <h2 className="text-5xl font-bangers text-black mb-8 tracking-wider drop-shadow-[2px_2px_0px_#fff]">The Mission!</h2>
        <div className="prose prose-slate max-w-none font-bold text-lg leading-relaxed text-black/80">
          <ReactMarkdown>{task.scenario}</ReactMarkdown>
        </div>
      </div>
      ...
      <div className="lg:w-1/2 p-8 lg:p-12 overflow-y-auto flex flex-col">
        ...
        <div className="mb-8 max-w-2xl mx-auto w-full">
          ...
          <div className="h-4 border-2 border-black bg-white rounded-full overflow-hidden shadow-[inset_0px_2px_0px_rgba(0,0,0,0.1)]">
            <div 
              className="h-full bg-[#0496FF] border-r-2 border-black transition-all duration-500"
        ...
        <div className="flex-1 max-w-2xl mx-auto w-full">
          <div className="retro-box p-8 relative bg-white">
            ...
            {q.type === "calculation" && q.options && (
              <div className="space-y-4">
                {q.options.map((opt) => (
                  <label
                    key={opt}
                    className={`flex items-center p-4 rounded-xl border-4 cursor-pointer transition-all ${
                      answers[q.id] === opt
                        ? "border-black bg-[#FFD166] shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] translate-x-1"
            ...
            {q.type === "multi-step" && (
              <div className="mt-6">
                <input
                  ...
                  className="w-full p-4 retro-input font-bold text-xl disabled:opacity-70"
                />
              </div>
            )}
            ...
            {q.type === "explanation" && (
              <div className="mt-6">
                <textarea
                  ...
                  rows={4}
                  className="w-full p-4 retro-input font-bold text-lg resize-none disabled:opacity-70"
                />
              </div>
            )}

            {currentResult && (
              <div className={`mt-8 p-6 border-4 border-black rounded-2xl flex items-start space-x-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${
